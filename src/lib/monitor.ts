...
export async function checkTarget(target: Target): Promise<{ status: number; latency: number; result: string; details?: string }> {
    ...
    try {
        ...
        // Best for WAFs that block bots. Works for kench.kr.
        const modernHeaders = {
        ...
        try {
            // First attempt: Modern headers, 15s timeout
            status = await performRequest(modernHeaders, 15000);

            // If the server rejects our "Modern" face with 400/403/406/500, it might be a legacy server.
            // goese.kr throws 400 Bad Request for modern headers.
            ...
            // Conditions to retry with "Legacy/Curl" mode
            const shouldRetry =
                msg === 'RETRY_LEGACY' ||
